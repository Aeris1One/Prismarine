...
browser, without stopping the java process, and go to http://localhost:7976 ,
...
The java process communicates with peers on port 7974 tcp by default. If you are
...
The user interface is available on port 7976. This port also accepts http API
...
In addition to the default user interface at http://localhost:7976 , the
following urls are available:

http://localhost:7976/test - a list of all available http API requests, very
...
http://localhost:7976/test?requestType=<specificRequestType> - same as above,
but only shows the form for the request type specified.

http://localhost:7976/doc - a javadoc documentation for client developers who
